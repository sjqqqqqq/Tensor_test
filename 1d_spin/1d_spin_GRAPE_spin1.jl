...
let
    # ── System parameters ────────────────────────────────────────────────────
    # Mixed-spin chain: site 1 is a SPIN-1 (the controlled site), sites 2..N are
    # SPIN-1/2.  Control field H_c(t) = c(t)·Sz^(1) acts on the spin-1 site only.
    N      = 11           # sites (1 spin-1 + (N-1) spin-1/2)
    Jx     = 2π           # XXX Heisenberg: Jx = Jy = Jz = 2π
    Jy     = 2π           # (set Jz = 2.2π for XXZ)
    Jz     = 2π
    T      = 10.0         # total time
    nsteps = 1000         # Trotter steps, dt = 0.01
    dt     = T / nsteps
    cutoff = 1e-10
    maxdim = 100
    ...
    # ── Site indices, states ──────────────────────────────────────────────────
    # Site 1 is "S=1" (m = +1,0,-1); sites 2..N are "S=1/2".
    s          = siteinds(n -> n == 1 ? "S=1" : "S=1/2", N; conserve_qns=true)
    # |ψ₀⟩    = |m=+1, ↓↓···↓↓⟩
    psi0       = MPS(s, vcat(["Up"], fill("Dn", N - 1)))
    # |ψ_T⟩   = |m=0, ↓···↓↑⟩   — same total-Sz sector as |ψ₀⟩ (see 1d_spin_spin1.jl)
    psi_target = MPS(s, vcat(["Z0"], fill("Dn", N - 2), ["Up"]))

    # ── Drift half-step gates (time-independent; computed once) ───────────────
    # Uniform spin-operator Heisenberg bond (valid for the mixed spin-1↔spin-1/2
    # bond on k=1):  Jx(Sx·Sx + Sy·Sy) + Jz·Sz·Sz
    # QN-safe form for Jx = Jy:  (Jx/2)(S+S- + S-S+) + Jz·Sz⊗Sz
    ...
    Sz1 = op("Sz", s[1])     # single-site control operator (spin-1 Sz)
    ...
    #   G_ctrl_n = exp(-im·dt·c_n·Sz^(1))
    #
    # Backward step n:  χ^{n-1} = P₁† · G_ctrl_n† · P₂† · χ^n
    #   P₂† ← apply(vcat(adj_odd_half,  adj_even_half), χ)
    #   P₁† ← apply(vcat(adj_even_half, adj_odd_half),  χ)
    #
    # Gradient formula (exact Trotter-GRAPE, one control site Sz^(1)):
    #   ∂F/∂c_n = 2·Re[ overlap* · (-im·dt) · ⟨λ_bc | Sz₁ | φ_n⟩ ]
    # where φ_n = G_ctrl_n · P₁ · ψ_{n-1}  (state AFTER ctrl gate, stored below).
    # (No factor of 2 vs. the Pauli-σz version: here the control couples to Sz,
    #  not σz = 2·Sz.)
    ...
    function forward(control)
        ...
        for n in 1:nsteps
            ...
            normalize!(psi)
            # Control gate: exp(-im·dt·c_n·Sz^(1))
    ...
    function compute_fg(control)
        ...
        for n in nsteps:-1:1
            ...
            # Step 2: ∂F/∂c_n = 2·Re[overlap* · (-im·dt) · ⟨λ_bc|Sz₁|φ_n⟩]
            ...
            chi = apply([exp(+im * dt * c_n * Sz1)], chi; cutoff, maxdim)
            normalize!(chi)
    ...
    # ── Initial pulse: warm-start from cached file if present, else random ────
    cache_file = joinpath(@__DIR__, "1d_spin_spin1.jld2")
    if isfile(cache_file) && length(load(cache_file, "control_opt")) == nsteps
        control0 = load(cache_file, "control_opt")
        println("Warm-starting from $cache_file (F=$(round(load(cache_file, "F_check"), digits=6)))."); flush(stdout)
    else
        Random.seed!(42)
        control0 = 0.5 * randn(nsteps)   # random initial pulse
        println("No usable cache; cold-starting from random pulse (seed 42)."); flush(stdout)
    end
    ...
    println("  Model  : XXX Heisenberg, spin-1 site 1 + $(N-1) spin-1/2, Jx=Jy=Jz=2π, T=$T, nsteps=$nsteps")
    println("  Task   : |m=+1,↓···↓⟩ → |m=0,↓···↓↑⟩ (single-quantum transfer to end)")
    println("  Method : L-BFGS (m=30), max 400 iterations")
    println(); flush(stdout)
    ...
    psi_opt = let psi = copy(psi0)
        for n in 1:nsteps
            ctrl = exp(-im * dt * control_opt[n] * Sz1)
            psi  = apply(vcat(odd_half, even_half, [ctrl], even_half, odd_half),
                         psi; cutoff, maxdim)
            normalize!(psi)
        end
    ...
    savefig(p1, "pulse_spin1.png")
    println("  Saved pulse_spin1.png")
    ...
    savefig(p2, "infidelity_time_spin1.png")
    println("  Saved infidelity_time_spin1.png")
    ...
    p3 = plot(1:length(F_history), 1.0 .- F_history;
        ...
        title="GRAPE Convergence  (N=$N, spin-1 site 1)",
        legend=false, lw=1.5, color=:darkorange,
        yscale=:log10, yminorgrid=true)
    savefig(p3, "convergence_spin1.png")
    println("  Saved convergence_spin1.png")

    # ── Save optimal pulse (enables warm-start on the next run) ───────────────
    jldsave(cache_file; control_opt, F_opt, F_check, N, T, nsteps)
    println("\n  Saved optimal pulse → $cache_file")
